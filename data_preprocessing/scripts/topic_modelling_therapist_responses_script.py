...
def main():
    ...
    # File paths
    therapist_topics_file = os.path.join(raw_data_dir, "therapist_topics_info.csv")
    prompt_file = os.path.join(prompts_dir, "therapist_response_topic_modelling_prompt.txt")
    output_file = os.path.join(processed_data_dir, "therapist_modelled_topics.csv")

    # Load data and prompt
    therapist_topics_data = pd.read_csv(therapist_topics_file)
    ...
    therapist_topics_data.to_csv(output_file, index=False)
